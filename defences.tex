...
The sybil attack clearly has a lot of consequences, some of them are severe and
...
also attempt to include all the seminal work.
...
\begin{tabular}{||l||l||c|c|c||c|c|c|c|c|c|c|c|c|c||}
  ...
  Year & Mechanism & \rot{Restrict sybil creation} & \rot{Perform sybil detection} & \rot{Restrict sybil participation} & \rot{Reputation system} & \rot{Certificate authority} & \rot{Resource testing} & \rot{Registration fee} & \rot{Network flow} & \rot{Random walk} & \rot{Community detection} & \rot{Content based} & \rot{Machine learning} & \rot{Miscellaneous} \\ \midrule \midrule
  ...
  2015 & EscapeLimit~\cite{gkorou2015trust}                   &           &           & $\bullet$ &           &           &           &           &           & $\bullet$ &           &           & $\bullet$ &           \\ \hline
...
\subsubsection{EscapeLimit}
EscapeLimit~\cite{gkorou2015trust} uses a variation of random walk - random walk
with restart to collect information in a P2P network and avoid sybils. That is,
there exist a probability where the random walk will restart from the initiator.
It does not require the existance of a social network, its assumption is that
there exist some quantifiable interaction between nodes. For example the number
of bytes exchanged in file sharing networks. Like the other mechanisms, it
assumes that the number of attack edges does not depend on the number of sybils.
Honest nodes act as initiators and perform EscapeLimit. The result is a
subjective graph which the honest nodes use to compute reputation scores of
other nodes. The restart behaviour restricts the initiator from ``walking'' into
the sybil region, thus the reputation score is unlikely to be influenced by the
sybils. The restart probability is set according to the expected length of hops
between nodes that interact with each other. Gkorou et el. found that to be 3
for real-world graphs and that translates to a restart probability of 0.25.
...
SmartWalk~\cite{liu2016smartwalk} which uses adaptive random walks. There are
two steps to SmartWalk. The first step uses machine learning to predict the
local mixing time given a node. The second step performs the random walk using
the local mixing time as the starting length, but on every hop the number of
hops remaining is also updated depending on the intermediate node's local mixing
