...
can be from direct experiences, or information obtained from someone else. The
...
labelled as an honest node if its random route intersects with the verifier's
random route. The number of accepted nodes for every intersection is limited by
a quota. The process is visualised in \autoref{fig:sybilguard}. Intuitively, the
