...
such that they \emph{eclipse} the victim from the rest of the network. The
victim can either be an identity or an object such as a key in DHT. The authors
mounted the latter variation, and they show it is possible to eclipse a key
using only 32 sybils in a DHT with 1.5 million users and 42,000 key.
...
On the other hand, a mailing list post by Mike Hearn suggest attackers
...
IP address of the attacker's sybils or ``rubbish'' addresses. This technique
...
51\% attack, where a single party gain complete control of the network. (3)
...
underlying requirements of the application. Some common tasks are data
...
sybils to claim that an honest node is misbehaving and have the other nodes expel
