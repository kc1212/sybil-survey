...
Decreasing the scalability even more. This exact attack was observed in 2015,
the attacker performed numerous small transactions which resulted in a queue of
88,000 transactions in the memory of many Bitcoin nodes, this then caused the
total number of Bitcoin nodes to drop by 10\% because some nodes ran out of
memory~\cite{bitcoinspam2015}. Fortunately, Bitcoin incorporates a transaction
fee, so spammers cannot abuse the network indefinitely~\cite{bitcoinspam}.
