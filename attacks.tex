...
originate from a single account, it is much more difficul to detect and prevent
when it is from a group of sybils. In the context of email, spam can be
...
The eclipse attack is closely related routing disruption, sybils in this case
are arranged in the network such that they \emph{eclipse} the victim from the
rest of the network. The victim can either be an identity or an object such as a
key in a DHT (distributed hash table)~\cite{singh2006eclipse}.
